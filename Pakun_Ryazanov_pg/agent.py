...
def DbJoin(dbname: str, user: str, password: str, host: str, port: int = 5432) -> Union[connection, None]:
    try:
...
class Agent:
    ...
    def Connections(self) -> None:
        ...
        if self.role == "Writer":
            self.conn2master = DbJoin(self.dbname, self.user, self.password, self.master)
            self.conn2slave = DbJoin(self.dbname, self.user, self.password, self.slave, 5433)
        else:
            for s in range(4):
                if self.master and self.conn2master is None:
                    self.conn2master = DbJoin(self.dbname, self.user, self.password, self.master)
                if self.slave and self.conn2slave is None:
                    self.conn2slave = DbJoin(self.dbname, self.user, self.password, self.slave)
